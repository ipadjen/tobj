...
//! ## Flat Data
//!
//! Values are stored packed as [`f32`]s (or [`f64`]s with the use_double feature) in flat `Vec`s.
//!
...
//!
//! ['use_double'] - Uses double-precision (f64) instead of single-precision (f32) floating points
...
/// stored packed as [`f32`]s  (or [`f64`]s with the use_double feature) in  flat `Vec`s.
///
