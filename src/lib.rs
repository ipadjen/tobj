...
//! ## Flat Data
//!
//! Values are stored packed as [`f64`]s in flat `Vec`s.
//!
...
/// stored packed as [`f64`]s in  flat `Vec`s.
///
...
fn parse_floatn(val_str: &mut SplitWhitespace, vals: &mut Vec<f64>, n: usize) -> bool {
    let sz = vals.len();
    ...
    // Require that we found the desired number of f64s.
    sz + n == vals.len()
}

/// Parse the float3 into the array passed, returns false if parsing failed
fn parse_float3(val_str: SplitWhitespace, vals: &mut [f64; 3]) -> bool {
...
#[cfg(feature = "merging")]
#[inline]
fn merge_identical_points<const N: usize>(points: &mut Vec<f64>, indices: &mut Vec<u32>)
where
    [(); size_of::<[f64; N]>()]:,
{
    if indices.is_empty() {
    ...
    let mut canonical_indices = HashMap::<[u8; size_of::<[f64; N]>()], u32>::new();

    let mut index = 0;
    *points = points
        .chunks(N)
        .filter_map(|position| {
            let position: &[f64; N] = &unsafe { *(position.as_ptr() as *const [f64; N]) };

            // Ugly, but f64 has no Eq and no Hash.
            let bitpattern =
                unsafe { std::mem::transmute::<&[f64; N], &[u8; size_of::<[f64; N]>()]>(position) };
